...
go() ->
    ...
    B1 = #rsc_meta{name="test book",type=node_type(book),description="just another test book",year=2010,url="github.org",pages_count=1234},
	
	RData1 = [],
	RData2 = [{authors,[0,1]},{disciplines,[0,1]},{files,[0]},{publishers,[0]}],
    
    {ok, C1ID} = meta_db:add_rsc(RootID,C1, RData1),
    {ok, R1ID} = meta_db:add_rsc(C1ID, B1, RData2),
...
%         author_id = 0,
%         publisher_id = 0,
%         description = "Эта книга посвящается, с уважением и любовью, духу, который живет внутри компьютера.
...
%     lists:foreach( fun(N) ->
%         metastore:put(#node{type = book}, book(), ParentID)
%         end,
%         lists:seq(1,Y)),
%     lists:foreach( fun(N) ->
%         Name = "depth_"++integer_to_list(Depth)++"_cat_" ++ integer_to_list(N),
%         metastore:put(#node{name = Name, type = category}, category(), ParentID)
%         end,
%         lists:seq(1,X)),
